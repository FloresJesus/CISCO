...
import AcademicosTab from "@/components/admin/reportes/academicos-tab"
...
export default function ReportesAcademicosPage() {
  ...
  const loadData = async (filters) => {
    ...
    try {
      ...
      const response = await api.get(`/admin/reportes/academicos?${params.toString()}`)
  ...
  const exportReport = async (format) => {
    try {
      setLoading(true)
      const response = await api.get(`/admin/reportes/academicos/export?format=${format}`, {
        responseType: "blob",
      })
      ...
      link.href = url
      link.setAttribute("download", `reporte-academico.${format}`)
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-dark flex items-center gap-3">
              <FiBarChart className="h-8 w-8 text-ciscoBlue" />
              Reportes Académicos
            </h1>
            <p className="text-gray-600 mt-2">Análisis del rendimiento académico y estadísticas de cursos</p>
          </div>
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="p-6">
            <AcademicosTab data={data} loading={loading} onFilterChange={handleFilterChange} />
          </div>
        </div>
      </div>
    </div>
