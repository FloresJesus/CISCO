...
export async function verifyStudentToken(request) {
  try {
    ...
    const [user] = await db.query(
      "SELECT id, email, rol FROM usuario WHERE id = ? AND rol = 'estudiante' AND activo = 1",
      [decoded.id]
    )
    ...
    console.error("Error al verificar token de estudiante:", error)
    return { success: false, error: "Error interno de autenticación" }
  }
}