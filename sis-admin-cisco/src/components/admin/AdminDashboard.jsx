...
import { 
  FiUsers, FiBookOpen, FiCalendar, FiDollarSign, 
...
export default function AdminDashboard() {
  ...
  // Datos simulados para gráficos y tablas
  const recentEnrollments = [
    { id: 1, student: 'María González', course: 'CCNA: Introduction to Networks', date: '2023-05-01', status: 'completada' },
    ...
    { id: 4, student: 'Juan López', course: 'CCNA: Switching & Routing', date: '2023-05-04', status: 'pendiente' },
    { id: 5, student: 'Pedro Sánchez', course: 'IoT Fundamentals', date: '2023-05-05', status: 'pendiente' },
  ];
  
  const upcomingCourses = [
    { id: 1, name: 'CCNA: Introduction to Networks', start: '2023-05-15', instructor: 'Roberto Méndez', enrolled: 18, capacity: 25 },
    { id: 2, name: 'CyberOps Associate', start: '2023-05-22', instructor: 'Laura Gómez', enrolled: 12, capacity: 20 },
    { id: 3, name: 'DevNet Associate', start: '2023-06-01', instructor: 'Miguel Ángel Torres', enrolled: 8, capacity: 15 },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {recentEnrollments.map((enrollment) => (
                <tr key={enrollment.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{enrollment.student}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                      ${enrollment.status === 'completada' ? 'bg-green-100 text-green-800' : 
                        enrollment.status === 'activa' ? 'bg-blue-100 text-blue-800' : 
                        'bg-yellow-100 text-yellow-800'}`}>
                      {enrollment.status.charAt(0).toUpperCase() + enrollment.status.slice(1)}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <a href={`/admin/academico/inscripciones/${enrollment.id}`} className="text-ciscoBlue hover:text-ciscoDarkBlue">Ver detalles</a>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
          <a href="/admin/academico/inscripciones" className="text-sm font-medium text-ciscoBlue hover:text-ciscoDarkBlue">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {upcomingCourses.map((course) => (
                <tr key={course.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <a href={`/admin/cursos/paralelos/${course.id}`} className="text-ciscoBlue hover:text-ciscoDarkBlue">Ver detalles</a>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="px-4 py-4">
        ...
        <div className="mt-1 flex items-center text-sm">
          ...
          <span className="ml-2 flex items-center text-xs font-medium">
            ...
            <span className={trendDirection === 'up' ? 'text-green-500' : trendDirection === 'down' ? 'text-red-500' : 'text-yellow-500'}>
              {trendText}
            </span>
          </span>
        </div>
